...
const StationCard = ({ station, index }: { station: any; index: number }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-0 flex flex-col sm:flex-row">
      <div className="w-full sm:w-[118px] h-[200px] sm:h-[183px] p-2">
        <img 
          src={imageUrl} 
          alt={station.name} 
          className="w-full sm:w-[102px] h-full sm:h-[164px] object-cover rounded-lg"
          onError={(e) => {
      ...
      <div className="flex-1 p-2">
        <div className="p-2">
          ...
          <button 
            ...
            className="w-full bg-[#3AC36C] text-white py-2 px-4 rounded-full text-sm font-bold mt-4 hover:bg-[#2ea85a] transition-colors active:scale-95"
          >
            Select Station
          </button>
        </div>
